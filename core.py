...
def get_video_resolutions(url):
    """
    Recupera le risoluzioni video disponibili per l'URL dato.
    Restituisce una lista di stringhe (es. ['1080p', '720p']).
    """
    ydl_opts = {
        'quiet': True,
        'no_warnings': True,
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            if 'entries' in info: # È una playlist
                # Prendiamo il primo video per determinare le risoluzioni (approssimazione)
                info = info['entries'][0]
                
            formats = info.get('formats', [])
            for f in formats:
                # Filtra solo video (che hanno altezza definita) e escludi 'none' o audio-only
                if f.get('vcodec') != 'none' and f.get('height'):
                    resolutions.add(f"{f['height']}p")
    except Exception as e:
        print(f"Errore nel recupero risoluzioni: {e}")
        return []

    # Ordina le risoluzioni in ordine decrescente numerico
...
def download_yt_video(url, playlist_name, progress_callback=None, resolution=None, download_path=None):
    """Scarica video o playlist in formato MP4 fino a 1080p o risoluzione specifica."""
    
    # Determina la cartella di destinazione
    if download_path:
        # Se specificato manualmente (da UI), usa quello direttamente
        # Nota: playlist_name viene appeso solo se è effettivamente una playlist o per organizzazione
        # Se l'utente sceglie una cartella specifica, forse vuole il video LI', ma per coerenza con
        # la logica precedente, creiamo una sottocartella col nome playlist/video
        video_dir = os.path.join(download_path, clean_filename(playlist_name))
    else:
        video_dir = os.path.join(get_download_path('video'), clean_filename(playlist_name))
        
    create_directory(video_dir)

    # Costruisce la stringa di formato
    if resolution:
        # Es. resolution = "720p" -> height = 720
        try:
            height = int(resolution[:-1])
            # Cerca video con altezza esatta, fallback sula migliore disponibile se non trovato (o errore, ma bestvideo gestisce)
            # bestvideo[height=...] prende l'esatta. Se non c'è, fallisce?
            # Meglio [height<=...]? No, l'utente ha scelto QUELLA.
            # Ma yt-dlp syntax: bestvideo[height=720]
        ...
        # Default behavior (limitato a 1080p come prima)
        format_str = 'bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]'
...
def download_yt_audio(url, playlist_name, progress_callback=None, download_path=None):
    """Scarica solo audio in formato MP3 (192 kbps)."""
    
    # Determina la cartella di destinazione
    if download_path:
        audio_dir = os.path.join(download_path, clean_filename(playlist_name))
    else:
